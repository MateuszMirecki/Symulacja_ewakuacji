    ...
    def move(self) -> None:
        ...
        if possible_steps:
            ...
            if field_value == 0:
                ...
                if hasattr(self.model, 'moves_this_step'):
                    self.model.moves_this_step += 1

    def remove_agent(self) -> None:
        self.grid.remove_agent(self)
        self.model.schedule.remove(self)
        if hasattr(self.model, "step_count"):  # Ensure step_count exists
            with open('statistics.txt', 'a') as stats_file:
                stats_file.write(f"Agent {self.unique_id} exited at step {self.model.step_count}\n")
